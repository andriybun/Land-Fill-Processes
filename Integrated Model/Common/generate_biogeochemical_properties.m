function properties_array = generate_biogeochemical_properties(spatial_params)
    ...
    properties_array_soa.effective_saturation = 0.6 * ones(sz);
    
    %% STUB:
    is_data_idx = (column_height_array > 0);
    properties_array_soa.compound_1_fraction = zeros(sz);
    properties_array_soa.compound_1_fraction(is_data_idx) = 0.5;
    properties_array_soa.compound_2_fraction = zeros(sz);
    properties_array_soa.compound_2_fraction(is_data_idx) = 0.3;
    %% END STUB
