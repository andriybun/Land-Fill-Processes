function out_flux = hydro_1d_2domain_exchange(flux_rate, t_idx, geometry, domain_params, t_params, log_normal_params)
    ...
    depth = 1:num_cells;
    exchange_rate_steps = zeros(geometry.num_domains, num_cells);
    ...
    % Normalize:
    sm = repmat(sum(exchange_rate_steps, 2), [1, num_cells]);
    ...
    out_flux(channel_domain_idx, t_idx:end) = out_flux(channel_domain_idx, t_idx:end) + ...
        in_flux(channel_domain_idx, :) .* dt .* log_normal_cdf(t, mu_steps(channel_domain_idx, num_cells), sigma(channel_domain_idx));
    %
    for cell_idx = 1:num_cells
        ...
        out_flux_cell = in_flux(matrix_domain_idx, :) .* dt .* ...
            log_normal_cdf(t, mu_steps(matrix_domain_idx, cell_idx), sigma(matrix_domain_idx));
        if cell_idx ~= num_cells
            for Xt_idx = 1:numel(t)
                ...
                out_steps_channel_out(Xt_idx:end) = out_steps_channel_out(Xt_idx:end) + ...
                    out_steps_dt_rep .* log_normal_cdf(t(Xt_idx:end) - t(Xt_idx), mu_steps_out(channel_domain_idx, cell_idx), sigma(channel_domain_idx));
            end
        else
